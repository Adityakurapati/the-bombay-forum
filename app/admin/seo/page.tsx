...
export default function SEOManagerPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface font-body text-on-surface">
      {/* Main */}
      <main className="min-h-screen bg-surface-bright flex flex-col flex-1">
        {/* Header */}
        <header className="px-6 md:px-12 py-6 md:h-20 bg-white flex flex-col sm:flex-row items-center justify-between sticky top-0 z-30 gap-4" style={{ boxShadow: '0px 20px 40px rgba(15,28,44,0.06)' }}>
          <h1 className="font-headline text-2xl md:text-3xl text-center sm:text-left" style={{ color: '#11262B' }}>SEO Manager</h1>
          <button
            className="w-full sm:w-auto text-white px-8 py-3 text-[10px] font-bold tracking-[0.2em] uppercase hover:opacity-90 transition-opacity font-label"
        ...
        <div className="px-6 md:px-12 py-8 md:py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {[
              ...
              <div key={card.label} className="bg-white p-6 border-l-4 border-primary" style={{ boxShadow: '0px 10px 20px rgba(15,28,44,0.03)' }}>
                ...
                <h2 className="font-headline text-3xl md:text-4xl text-on-surface mb-1">{card.value}</h2>
          ...
          {/* Tabs */}
          <div className="flex gap-0 border-b border-outline-variant/20 mb-8 overflow-x-auto whitespace-nowrap custom-scrollbar">
            {tabs.map((tab) => (
              <button
                key={tab}
          ...
          {/* Pages Table */}
          <div className="w-full bg-white mb-12 overflow-x-auto custom-scrollbar border border-outline-variant/10" style={{ boxShadow: '0px 20px 40px rgba(15,28,44,0.04)' }}>
          ...
          <section className="mb-20">
            ...
            <div className="overflow-x-auto custom-scrollbar border border-outline-variant/10" style={{ boxShadow: '0px 20px 40px rgba(15,28,44,0.04)' }}>
              <div className="min-w-[800px]">
                ...
                <div className="bg-white divide-y divide-outline-variant/10">
                  {keywords.map((kw, i) => (
        ...
        {/* Footer */}
        <footer className="mt-auto bg-surface-container-highest flex flex-col items-center py-12 px-6 md:px-12 space-y-6 w-full text-center">
          <Link href="/">
            <div className="font-headline text-xl md:text-2xl font-bold tracking-tighter text-brand-navy">
